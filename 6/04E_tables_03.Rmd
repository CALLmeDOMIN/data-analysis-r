...
df_multi <- data.frame(
  ID = 1:5,
  Midterm_Score = c(75, 88, 94, 83, 78),
  Final_Score = c(85, 92, 88, 90, 80),
  Midterm_Grade = c("B", "A", "A", "B", "C"),
  Final_Grade = c("B+", "A-", "B", "A-", "B-")
)

df_multi %>%
  kable("html", caption = "Multi-level Header Example") %>%
...
kable(iris[1:10, ], "html", caption = "Iris Dataset Sample") %>%
  kable_styling(bootstrap_options = c("striped", "hover")) %>%
  pack_rows("Setosa", 1, 3) %>%
  pack_rows("Versicolor", 4, 7) %>%
  pack_rows("Virginica", 8, 10) %>%
  footnote(
    general = "This is a sample of the iris dataset.",
    general_title = "Note:"
  )
```
...
mtcars %>%
  ...
  gt() %>%
  tab_header(title = "Average MPG by Cylinders") %>%
  fmt_number(
    columns = vars(avg_mpg),
    decimals = 1
  ) %>%
  data_color(
    columns = vars(avg_mpg),
