...
Data **tidying** is a fundamental step in the data analysis process. Ensuring that your data is in a **consistent, tidy format** makes it easier to manipulate, visualize, and model. The **`tidyr`** package, part of the **tidyverse**, provides a suite of functions designed to help you **transform messy data** into tidy data.
...
Before diving into data tidying, ensure the necessary libraries are installed and loaded. We will use:
...
Tidy data follows a specific structure that makes data analysis more straightforward. According to **Hadley Wickham**, there are three interrelated rules that make a dataset tidy:
...
The `tidyr` package offers several functions that help **reshape** and **tidy** data. The **core** functions include:
...
A more challenging scenario arises when column names contain **multiple variables**. For example, the `who2` dataset:

```{r}
who2
```

**`who2`** records tuberculosis data. There are columns `country` and `year`, plus ~56 columns like `sp_m_014`, `ep_m_4554`, `rel_m_3544`, etc. Each column name is made of three pieces separated by `_`:  
1. The method of diagnosis (`sp`, `rel`, `ep`).  
2. The gender (`m` or `f`).  
3. The age range (`014`, `1524`, `2535`, `3544`, `4554`, or `65`).

Hence we have multiple variables embedded in the column name. We can fix this with:
...
Another advanced scenario is when column names mix variable names **and** variable values. For instance, the `household` dataset might have columns like `name_1`, `dob_1`, `name_2`, `dob_2`, which hold the **child** number (1 or 2) plus the type of data (`name`, `dob`). We can parse that with `names_to = c(".value", "child")`:
...
`pivot_wider()` is the inverse of `pivot_longer()`. It spreads key-value pairs across multiple columns, going from a **long** to a **wide** format.
...
Might have columns like `org_id`, `measure_cd`, and `prf_rate`. We can pivot so each measure code becomes a separate column:
...
`separate()` splits a single column into multiple columns based on a separator. By default, it splits on **non-alphanumeric** characters, but you can specify `sep`.
...
`unite()` is the opposite of `separate()`. It combines multiple columns into a single column by concatenating their string values.
...
df_dates <- tibble(
  datetime = c("2023-01-01 12:00:00", "2023-01-02 13:30:00", "2023-01-03 14:45:00")
)
```
...
df_fully_complete <- df_complete %>%
  complete(id, category, fill = list(value = 0))
...
df_multi <- tibble(
  type_gender_age = c("A_m_25", "B_f_30", "C_m_35", "D_f_40")
)
...
df_separated_new <- df_multi %>%
  separate(type_gender_age, into = c("type", "gender", "age"), sep = "_")
...
**Question:** Using the `flights` dataset from `nycflights13`, perform the following steps: 
1.
...
res <- flights %>%
  ...
  mutate(
    dep_hour = ifelse(!is.na(dep_time), sprintf("%02d", dep_time %/% 100), "00"),
    dep_minute = ifelse(!is.na(dep_time), sprintf("%02d", dep_time %% 100), "00"),
    arr_hour = ifelse(!is.na(arr_time), sprintf("%02d", arr_time %/% 100), "00"),
    arr_minute = ifelse(!is.na(arr_time), sprintf("%02d", arr_time %% 100), "00")
  ) %>%
  unite(departure_time, dep_hour:dep_minute, sep = 'h', remove = TRUE) %>%
  unite(arrival_time, arr_hour:arr_minute, sep = 'h', remove = TRUE) %>%
  select(year, month, day, departure_time, arrival_time, carrier, tailnum)
