...
```{r}
# Your code here:
my_list <- list(numbers = c(10, 20, 30, 40, 50),
...
my_list$df_sample <- cbind(my_list$df_sample, hp = mtcars$hp)
```
...
nested_li <- list(my_list, list(log_vec))
str(nested_li)
```
...
```{r}
# Your code here:
df_additional <- data.frame(Name = c("1", "2", "3"), Age = c(1, 2, 3))
additional_li <- list(matrix(1:9, 3, 3), df_additional)
str(additional_li)
```
